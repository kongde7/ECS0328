...
public class Resource
{
	...
	public void releaseOne( int k, Release rel, ArrayList<VM> vmList, ArrayList<NC> ncList, Table table, Times times ) throws IOException
	{
		NC nc = null;
		VM vm = null;
		int i, j;
		...
		{
			vm = vmList.get(i);
			if( rel.index[k].contentEquals( vm.vmId ) )
			{
				//到NC表中返还CPU和内存
				for ( j = 0; j < ncList.size(); j++ )
				{
